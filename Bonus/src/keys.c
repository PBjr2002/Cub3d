...
/*   Updated: 2025/01/06 18:04:18 by pauberna         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_key_push(int keycode, t_data *model)
{
	if (keycode == ESC_KEY)
		...
		model->keys[5] = 1;
	else if (keycode == E_KEY && model->keys[6] == 0)
		model->keys[6] = 1;
	else if (keycode == R_KEY && model->keys[7] == 0)
		return (model->keys[7] = 1, \
		mlx_mouse_show(model->mlx, model->mlx_window), 0);
	else if (keycode == R_KEY)
		return (model->keys[7] = 0, \
		mlx_mouse_hide(model->mlx, model->mlx_window), 0);
	return (0);
}
...
int	ft_key_release(int keycode, t_data *model)
{
	if (keycode == W_KEY && model->keys[0] == 1)
		...
		model->keys[5] = 0;
	else if (keycode == E_KEY && model->keys[6] == 1)
		model->keys[6] = 0;
	return (0);
}
